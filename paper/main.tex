...
\section{Introduction}
    ...
    The use of a network to model a complex system is often carried out with one of two aims: modeling dynamics \textit{on} a network or modeling the dynamics \textit{of} a network. This work falls into the latter category — we model the growth of a complex system via the growth of its network representation. Specifically we focus on a type of complex system called a higher-order network and a type of network representation called a hypergraph.

    Higher-order networks (HONs) are complex systems composed of entities and the multi-way relationships between them. Many real-world systems are composed of higher-order interactions. Social interaction (both in-person \cite{chodrowGenerativeHypergraphClustering2021} and online \cite{bensonSimplicialClosureHigherorder2018}), academic co-authorship \cite{wu2022hypergraph}, and ecological systems \cite{levine2017beyond} all contain interactions consisting of more than two entities.

    It is often useful to represent an HON using a data structure that models its interactions. Hypergraphs, bipartite graphs, and simplicial complexes can all be used to model HONs \cite{battistonNetworksPairwiseInteractions2020}. Dyadic networks, or graphs, can be used to model dynamics on HONs when the equations of the dynamics properly account for multiway interaction \cite{peixoto2026graphs}. However, when multiway interactions are not incorporated into the model specification, down-the-line analyses suffer \cite{bensonSimplicialClosureHigherorder2018, klamt2009hypergraphs}.
...
\subsection{Generative Models for Inference and Community Detection}
    ...
    Graph generative model for community detection: \cite{newmanStochasticBlockmodel} 
...
\section{Model Description}
    ...
    \end{itemize}
    In our simulations we initialize $\at{H}{0}$ with a single edge containing one node of each label, but any combination of hypergraph and node labels can be used as an initial condition.
...
\subsection{Asymptotics} \label{sec:asymptotics}
    ...
    \begin{figure*}[t]
        ...
        \caption{Dynamics! Top shows the three leading eigenvectors of the distribution $\at{\jointdist}{\infty} \paren{k_0, k_1}$. Bottom left gives the cosine similarity (normalized projection) between one run of simulation dynamics and these eigenvectors. Bottom right gives the final simulation state.
        }
...
\subsubsection{Degree Distribution}
    ...
    Our derivation follows that of \cite{heEdgeCorrelationsLink2025}, which in turn is based on a derivation by \cite{mitzenmacherBriefHistoryGenerative2004}. 
    ...
    Under this assumption, it is possible to show that the degree distribution follows a power law with exponent $\zeta = 1 + \frac{\gamma}{\tilde{\sigma}}$, where 
    \begin{align}
        \tilde{\sigma} = \frac{1 + \eta_+(\rho_{00} + \rho_{11} - 1) + 2\eta_- \rho_{01}}{\brackets{d}} \\
        \rho_{ij} = \brackets{\frac{k_i k_j}{k}} \;. 
    \end{align}
    In the expression for $\rho_{ij}$, the expectation is taken with respect to the stationary joint distribution $\at{\jointdist}{\infty} \paren{k_0, k_1}$.
    \begin{figure}[t]
        \centering
        \includegraphics[width=0.5\textwidth]{../fig/degree_distribution.png}
        \caption{
            Degree distribution from simulation (points) and theoretical power law (line) with exponent computed from model parameters and joint label distribution moments. 
...
\section{Appendix}
    ...
    \subsection{Derivation of the Stationary Joint Distribution of Labels in Edges}

    \subsection{Derivation of the Degree Distribution Power Law Exponent}
    ...
    Define the moments 
    \begin{align}
        \rho_{00} &= \brackets{\frac{k_0^2}{k}} \quad \text{,} \quad \rho_{11} &= \brackets{\frac{k_1^2}{k}} \quad \text{and} \quad \rho_{01} = \brackets{\frac{k_0 k_1}{k}} \;.
    \end{align}
    ...
    \begin{align}
        \sigma_0 \triangleq \brackets{\frac{k_0}{k}\paren{1 + \eta_+(k_0 - 1)} + \frac{k_1}{k} \eta_- k_0} = 1 + \eta_+ \squarebrackets{\rho_{00} + \rho_{11} - 1} + \eta_- \rho_{01}\;. 
    \end{align}
    Similarly, the expected number of nodes of label 1 copied into the new edge $e$ from $f$ is 
    \begin{align}
        \sigma_1 \triangleq \brackets{\frac{k_1}{k}\paren{1 + \eta_+(k_1 - 1)} + \frac{k_0}{k} \eta_- k_1} = 1 + \eta_+ \squarebrackets{\rho_{00} + \rho_{11} - 1} + \eta_- \rho_{01}\;. 
    \end{align}
    We let 
    \begin{align}
    ...
    Let $\tilde{\sigma} = \frac{\sigma}{\brackets{d}}$, where $\brackets{d}$ is the average degree of a node in the hypergraph given by \Cref{eq:average-degree}. 
    
    We make a mean-field assumption in which the degree $d$ of a node is independent from the counts $k_0$ and $k_1$ of labels in an edge containing that node. 
    Under this assumption, the probability that a single node selected in the sampling step has degree $d$ is $dp_d^{(t)}$, where $p_d^{(t)}$ is the fraction of nodes with degree $d$ at time $t$, since that node participates in $d$ edges.
    The expected number of nodes of degree $d$ sampled in the edge-sampling step is therefore $\tilde{\sigma} d p_d^{(t)}$, and the expected \emph{change} in the count of degree $d$ nodes through the edge-sampling step is 
    \begin{align}
    ...
    \begin{widetext}
        \begin{align}
            n^{(t+1)}p_{d}^{(t+1)} - n^{(t)}p_{d}^{(t)} = \tilde{\sigma} \squarebrackets{(d-1)p_{d-1}^{(t)} - d p_d^{(t)}} + \lambda (p_{d-1}^{(t)} - p_d^{(t)}) \;.
        \end{align}
    \end{widetext}
    ...
    \begin{widetext}
        \begin{align*}
            p_d \gamma  = \tilde{\sigma} \squarebrackets{(d-1)p_{d-1} - d p_d} + \lambda (p_{d-1} - p_d) \;.
        \end{align*}
    \end{widetext}
    Rearranging, we obtain 
    \begin{align*}
        \frac{p_d}{p_{d-1}} &= \frac{\tilde{\sigma}(d-1) + \lambda}{\tilde{\sigma} d + \gamma + \lambda} \\ 
        &= 1 - \frac{\tilde{\sigma} + \gamma + \lambda}{\tilde{\sigma} d + \gamma + \lambda}
    \end{align*}
    Allowing $d$ to become large (which describes the tail of the degree distribution), we have
    \begin{align*}
        \frac{p_d}{p_{d-1}} &\approx 1 - \frac{\tilde{\sigma} + \gamma + \lambda}{\tilde{\sigma} d} \;,
    \end{align*}
    a relation which corresponds to a power law with exponent $\zeta = 1 + \frac{\gamma}{\tilde{\sigma}}$. 
    Explicitly calculating this moment requires computing the moments $\rho_{00}$, $\rho_{11}$, and $\rho_{01}$, which can be done computationally from the stationary joint distribution $\jointdist \paren{k_1,k_2}$. 
