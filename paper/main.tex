...
\Cref{eq:likelihood} defines a likelihood function over both the parameter vector $\paramvector$ and the node labels $\vz$.
    ...
    We therefore use an intersection-based approximation to the likelihood of forming edge $e$ in which we only sum over candidate copied edges $f$ such that $f = \argmax_{f' \in \edges} \abs{f' \cap e}$.
    ...
    \phil{Need to give formulae for the change in likelihood under a proposed change to $\vz$ in the simulated annealing procedure, talk about the novel approach to acceptance probability and describe algorithm.}
...
\begin{figure*}
    ...
    \caption{Community detection results on synthetic data. 
    This figure will make more sense and will hopefully tell a story once we get some compute.} \label{fig:community-synthetic}
\end{figure*}
