...
\appendix
    ...
    \section{Approximation Argument for Simulated Anealing}\label{sec:simulated-annealing-approximation}
        ...
        For convenience, we define $\prec$ such that $\seededge\prec\newedge$ is true if and only if $\seededge$ is a valid candidate parent edge, which is rigorously defined as follows.
        \begin{align}
            \seededge \prec \newedge \iff & \exists t \geq 1, t \in \mathbb{Z} | \seededge, \newedge \in \edges^{t-1}, \\ & \seededge\notin \edges^t, \newedge\in \edges^t, \seededge \cap \newedge \neq \{\emptyset\}
        \end{align} 
