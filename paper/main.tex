...
\appendix
    ...
    \section{Algorithmic specification for simulated annealing}
        ...
        \phil{Some things to check here: }
        \begin{itemize}
            \item \phil{It looks like $\sigma_{\hypergraph}$ is only computed after the first epoch, but it is used in the acceptance probability for all epochs. Is this intentional?}
            \item \phil{What is $A$?}
            \item \phil{Should require the number of steps or epochs desired. Maybe $A$ is the number of epochs? }
        \end{itemize}
        Here, 
        \begin{align}
