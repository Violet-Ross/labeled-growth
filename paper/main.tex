...
\violet{Look over this paragraph}
The SEM algorithm continues iterating until the stopping criterion is met. 
In our experiments, the stopping criterion is met when each of the current parameter estimates in $\at{\hat{\paramvector}}{\ell}$ is ``close'' to all of the 800 preceding estimates of the corresponding parameter. 
We define ``close'' using both a relative and absolute tolerance. 
A parameter estimate $\hat{\copyrate_+}^{(\ell)}$ is close to the preceding estimate $\hat{\copyrate_+}^{(\ell - 1)}$ if $|\hat{\copyrate_+}^{(\ell)} - \hat{\copyrate_+}^{(\ell - 1)}| \leq \text{tol}_{(\text{absolute})} + \text{tol}_{(\text{relative})} * \hat{\copyrate_+}^{(\ell - 1)}$.
The absolute tolerance ensures that the criterion can be met even when the values of the estimates are near zero.
For all runs of SEM we used $\text{tol}_{(\text{absolute})} = 0.01$ and $\text{tol}_{(\text{absolute})} = 0.015$.
...
\phil{Violet, we need to say something about why this might be, or sorta acknowledge the mystery of it. Thoughts?}
...
\appendix
    ...
    \subsection{Implementation Details}
        ...
        We used an exponentially decaying learning rate for SEM, which has the form $\lr^{(t)} = \lr^{(0)} \cdot e^{-ct}$.
        The initial learning rate was $\lr^{(0)} = 0.01$ and the exponent was $c = 0.001$.
