...
\begin{figure}
    \includegraphics[width=0.48\textwidth]{../fig/community-structure-sims-combined.png}
        \caption{
            ...
            \phil{Fig is good but maybe we want to do $10^3$ reps just to reduce the noise a bit, and possibly increase the resolution of the copy parameters.}
...
\begin{table*}
    \caption{Data sets used for parameter estimation. We show the number of nodes ($n$), edges ($m$), average degree ($\langle d \rangle$), and average edge size ($\langle k \rangle$) for each dataset.}\label{tab:datasets}
        ...
        \texttt{high-school} \cite{gemmetto2014mitigation,stehle2011high} & 320 & 60,544 & 484.6 & 2.6 & Students & Social interaction & Gender  \\
        \texttt{primary-school} \cite{mastrandrea2015contact} & 227 & 44,626 & 590.7 & 3.0 & Students & Socail interactions & Gender \\
...
The coauthorship data, meanwhile, falls on the line of equality for the edge copy and external node addition mechanisms, but there is a difference in the novel node addition parameters: $\hat{\same{\novelrate}} > \hat{\opp{\novelrate}}$.
...
The senate-bills data appears to have slight homophily in the edge copy and external node addition steps, and has near-zero values for the novel node addition parameters.
The house-bills data also has near-zero valued novel node addition parameters and has similar edge copy parameters to senate-bills (though house-bills is slighly heterophilic and senate-bills is slightly homophilic).
The main distinction between the parameters of the two congressional networks is the external node addition step, where the values of both $\extantrate_+$ and $\extantrate_-$ are higher in house-bills than in senate-bills.
This might be explained by the higher mean edge size of house-bills (20.5, as opposed to the 8.0 of senate-bills.). 
That is, a bill in the House of Representatives has on average more cosponsors than a bill in the Senate, and since only about half of $\seededge$ is copied to $\newedge$, and $\novelrate_-$ and $\novelrate_+$ are close to zero, the external node parameters will grow proportionally to the mean edge size.
...
Initial sufficient statistics $\sufficientstatsvec^{(0)} = (1, 2, 1, 2, 0.5, 0.5, 0.5, 0.5)$ were used.
...
Since in data analysis contexts the labels $\vz$ (and therefore parameter estimates $\hat{\paramvector}$) are not available, we instead use a single, fixed parameter vector for each of these three experiments: $\hat{\paramvector} = (.9, .1, 1, .25, .001, .001)$.
...
\begin{figure}
    ...
    \caption{Community detection results on empirical datasets.  Box plots correspond to the max likelihood label set's corresponding ARI plotted for 10 runs of simulated annealing on each dataset.  We compare these results with the ARI of the labels generated from the spectral clustering and greedy modularity algorithms of community detection.}
