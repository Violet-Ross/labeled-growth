...
\subsection{Asymptotic Properties} \label{sec:asymptotics}
    ...
    \Cref{fig:dynamics-eigenvectors} shows that, when $\eta_+$ is sufficiently large, the stationary distribution $\at{\jointdist}{\infty} \paren{k_0, k_1}$ is concentrated on edges in which typical edges have large majorities of nodes with the same label (panel b), thus displaying a form of higher-order homophily \cite{laber2026guide}. 
    ...
    When $\eta_+$ is sufficiently large, the spectral gap $1 - \lambda_2$ is small (panel a), indicating that this transient decays slowly and that the system may be dominated by edges with a single majority label for relatively long periods of time before equilibrating to the leading eigenvector (panels d and e).
...
We compare these methods against each other and against a baseline spectral clustering method applied to the clique projection of the hypergraph.
...
\appendix
    ...
    \section{Simulated Annealing Illustration}
        ...
        \begin{figure*}
            ...
            \caption{
                Simulated annealing for community detection in a hypergraph generated from our model.
            }
        \end{figure*}
