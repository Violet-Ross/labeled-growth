...
\date{\today}
\begin{abstract}
    There are many extant models of hypergraphs with interactions governed by some form of homophily or assortativity, but almost all of these assume independence between edges conditional on node parameters. 
    Relaxing this strong assumption, we study a mechanistic model of growing hypergraphs in which edge formation is influenced by the realization of previous edges and is tuned by homophily between binary node attributes. 
    ...
    Our findings highlight the benefits of moving beyond edge-independence assumptions in modeling and data analysis, and point to several directions of future work. 
\end{abstract}
...
\phil{Must make parameter names consistent between text and figures.}
...
\section{Introduction}
    ...
    We also describe a community detection algorithm for estimating node labels via simulated annealing on the \model{} likelihood. 
...
\section{\model{} Model Description} \label{sec:model-description}


    The \Model{} (\model{}) is a generative model for growing hypergraphs with binary node labels. 
    \model{} is an extension of the Hyperedge Copy Model (HCM) of He et al. \cite{heEdgeCorrelationsLink2025}. 
    In their HCM, edges form through a combination of three distinct steps: a copy step, in which a previous edge is sampled and imperfectly copied; an extant node addition step, in which new nodes are added to the new edge from the existing node set; and a new node addition step, in which new nodes are added to the new edge from outside the existing node set.
    \model{} replicates these mechanisms but adds label-aware parameters to regulate each. 

    Formally, a \model{} state at time $t$ is a hypergraph $\at{\hypergraph}{t} = \paren{\at{\nodes}{t}, \at{\edges}{t}}$, where $\nodes$ is the set of nodes and $\edges$ is the set of hyperedges, as well as a binary label vector $\vz \in \{0, 1\}^{|\at{\nodes}{t}|}$ which assigns to each node a label of either 0 or 1.
    We let $\bar{z} = 1 - z$ denote the opposite label of $z$.
    At timestep $t$, we add edge $\newedge$ to $\at{\edges}{t}$ to obtain $\at{\edges}{t+1}$, where $\newedge$ is formed according to the following three-step process:
    \begin{itemize}
        \item 
...
\subsection{Asymptotic Properties} \label{sec:asymptotics}
    ...
    Many asymptotic properties of our \model{} model can be described in terms of the joint distribution $p_{K_0, K_1}(k_0, k_1)$ of nodes of labels 0 and 1 in a uniformly random edge. 
    This joint distribution can be obtained as the leading eigenvector of a linear map, as we now describe. 
    ...
    The resulting final simulation state matches the stationary distribution $\at{\jointdist}{\infty} \paren{k_0, k_1}$ relatively closely (panel (e)).
    ...
    The second eigenvector $v_2$ (panel c), however,  is antisymmetric, describing a transient in which one a single label group tends to dominate most edges. 
...
\subsubsection{Degree Distribution} \label{sec:degree-distribution}
    ...
    The degree distribution of a hypergraph generated from the \model{} model follows an asymptotic power law with an exponent which can be computed in closed form from the model parameters and the stationary joint distribution $\at{\jointdist}{\infty} \paren{k_0, k_1}$. 
...
\violet{Look over this paragraph}\phil{Made revisions. Happy with it but need a small comment to say why 800 (and why not 100, or 1,000, which would have been obvious default choices).}
...
\subsection{Community Detection via MLE} \label{sec:community-detection}
    ...
    We'll argue that, when the number of nodes $n$ is large, $p(\newedge|\seededge, \focalnode, \vz, \paramvector) \in O(n^{-x})$, which implies that edges $\seededge$ which do not maximize $\abs{\seededge \cap \newedge}$ will have negligible contributions to the likelihood.
    The essence of the argument is that forming $x$ extant nodes requires $x$ independent samples from the sets $\nodes_0\setminus \seededge$ and $\nodes_1\setminus \seededge$, each of which have size scaling linearly with $n$. 
    ...
    In each step, we propose a change to the labels $\vz$ by generating a node index $j$ uniformly at random and evaluating the change in likelihood of "flipping" the binary node label ($z_j \gets 1 - z_j$).  
    \phil{We need to at least be able to put \Cref{eq:sa-approx} and say what it means in the main text; the justification can stay in the appendix.}
    ...
    When evaluating this change in likelihood, we implement custom data structures that allow us to compute only the change in likelihood from the prior state by storing the components of the prior likelihood calculation 
    This results in a roughly 30-fold reduction in compute time on our empirical data sets (described in \Cref{sec:experiments}) at the expense of increasing memory requirements. 
    
    \phil{Haven't done a style pass over the below yet.}
...
\begin{figure*}[t]
    ...
    \caption{
        ...
        For each set of parameters, 20 indepedent 2000-edge hypergraphs are generated according to the model and SEM is performed independently on each. \phil{Violet, for how many simulation steps?}
...
Due to the high temporal resolution of these data, we consider a sequence of interactions to correspond to a single hyperedge of those interactions are identical and temporally contiguous. 
...
Then reciprocity of bipartisan cosponsorship suggests that the Republican cosponsors of $\seededge$ may also cosponsor $\newedge$ to ``return the favor.''
...
\phil{This seems helpful but I think we still owe an explanation if possible for why house and senate might be different. Possibly here? \cite{harbridge2023bipartisan}.}
...
These experiments do suggest, however, that there exist classes of hypergraphs in which community structure can be more reliably detected via likelihood maximization under \model{} than at least this standard graph method. 
...
We compare \model{} SEM against several competitors. 
...
\begin{figure}
    ...
    Box plots describe the distribution of the ARI of \model{} SEM on the selected data set across 10 separate runs. 
...
<<<<<<< HEAD
We show our results in \Cref{fig:community-empirical}.
We observe that across each of these three data sets, none of the compared methods consistently outperforms the others; this is directionally aligned to ``no free lunch'' results in community detection \cite{peel2017ground}. 
\model{} SEM is perhaps the most consistent of the methods, achieving relatively high ARIs and not failing completely on any of the three data sets. 
Hypergraph modularity maximization is outperformed by both \model{} SEM and nonbacktracking spectral clustering on \texttt{senate-bills}. 
Hypergraph modularity maximization was reported to achieve an ARI of 1.0 on both \texttt{primary-school} and \texttt{high-school}, reflecting perfect recovery, but this result cannot be directly compared to other since this required nonbinary partitions. 
Nonbacktracking spectral clustering achieves the highest of all ARIs on \texttt{senate-bills} and is competitive on \texttt{primary-school}, but appears to completely fail on \texttt{high-school}. 
The two dyadic methods perform similarly to each other throughout, failing on \texttt{senate-bills} and achieving relatively high ARIs on \texttt{primary-school} and \texttt{high-school}. 
=======
...
This is because the baseline methods, greedy modularity and spectral clustering, are frequently justified via the stochastic block model \cite{lei2015consistency, newman2016equivalence}, which relies on the assumption of edge indpenece, where our method explicitly utilizes dependent hyperedges.
...
>>>>>>> 6957237ca39485658bd5a449ad4227ae3c21cf5e
...
\section{Discussion}
    ...
    Developing more efficient heuristics for maximizing the likelihood over $\vz$ will be an important direction for scalable application the \model{} model for community detection. 
    Scalability may also imply opportunities for improved performance.
    We note, for example,  that \cite{chodrowNonbacktrackingSpectralClustering2023} proposed an algorithm which obtained an ARI of approximately 0.25 on the \texttt{senate-bills} data set, higher than our proposed simulated annealing method. 
    ...
    A final possible extension is to incorporate recency-bias \cite{bensonSequencesSets2018} into the copy mechanism, so that more recent edges are more likely to be copied while edges which are sufficiently old may cease to be copied at all. 
...
\section*{Data Availability}
    ...
    \begin{itemize}
        \item \texttt{senate-bills}: 
        \item \texttt{house-bills}:
        \item \texttt{primary-school}: 
        \item \texttt{high-school}: 
        \item \texttt{coauthorship}: 
        \item \texttt{enron-emails}:
    \end{itemize}
...
\appendix
    ...
    \subsection{Implementation Details}
        ...
        We use an exponentially decaying learning rate for SEMwith form $\lr^{(t)} = \lr^{(0)} \cdot e^{-ct}$.
        The initial learning rate is $\lr^{(0)} = 0.01$ and the exponent is $c = 0.001$.
