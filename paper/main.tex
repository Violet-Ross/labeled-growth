...
\begin{abstract}
    ...
    We also demonstrate an approach to community detection via simulated annealing which, though computationally expensive, achieves competitive results on certain data sets known to be challenging to standard community detection techniques derived from models that assume edge independence.
...
\subsection{Community Detection via Simulated Annealing} \label{sec:community-detection}
    ...
    To run simulated annealing for a single epoch, we perform $n$ individual update steps. 
    In each step, we propose a change to the labels $\vz$ by generating a node index $j$ uniformly at random (without replacement) and evaluating the change in likelihood of "flipping" the binary node label ($z_j \gets \bar{z}_j$).  
    \phil{Frannie, is the ``without replacement'' correct here?}
    If the proposal increases $\tilde{\cL}(\vz)$, it is accepted.
    If the proposal decreases $\tilde{\cL}(\vz)$, however, we may also accept it with an acceptance probability $a(\ell)$ that decreases with the epoch $\ell$.
    ...
    We use a custom acceptance schedule $a(\ell)$ for our simulated annealing implementation.
    ...
    In the first epoch, we accept \emph{all} proposals, while storing the changes the objective $\tilde{\cL}$ produced by those proposals. 
...
\appendix
    ...
    \section{Simulated Annealing for Community Detection}\label{sec:simulated-annealing-algorithm-spec}
        ...
        \phil{Frannie, is something a bit off in this algorithm? If I understand correctly $B$ should be an evaluation of a normal quantile (i.e. the value of the CDF) rather than a simple from a normal distribution. 
        That way it's guaranteed to be a probability and therefore comparable with $x$. Am I missing something?}
    ...
    \section{Simulated Annealing Illustration}
        ...
        \begin{figure*}
            ...
            \caption{
                ...
                Dashed line on the right shows the ARI achieved by Laplacian spectral clustering applied to the weighted dyadic graph projection.  
            }
        \end{figure*}
