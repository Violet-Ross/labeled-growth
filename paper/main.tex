...
\begin{abstract}
    ...
    Relaxing this strong assumption, we study a mechanistic model of growing hypergraphs in which edge formation is influenced by the realization of previous edges and is tuned by homophily between binary node labels. 
    ...
    We also demonstrate an approach to community detection via simulated annealing which, though computationally expensive, achieves competitive results on synthetic data generated from our model and certain empirical data sets known to be challenging to standard community detection techniques derived from models that assume edge independence.
...
\section{Introduction}
    ...
    \frannie{Per existing comment, more higher order here}
    ...
    \end{align}
    Many generative models for community detection fall into this category, including hypergraph stochastic block models \cite{chodrowGenerativeHypergraphClustering2021,ruggeriCommunityDetectionLarge2023,kim2018stochastic} and some generalizations \cite{badalyanStructureInferenceHypergraphs2024,hood2026broad}.
    ...
    In this paper, we introduce a model of binary node-labeled hypergraphs in which edges form via a noisy copying interaction, which depends on the labels. 
...
\section{\model{} Model Description} \label{sec:model-description}
    ...
    \Model{} (\model{}) is a generative model for growing hypergraphs with binary node labels. 
    ...
    \end{itemize}
    \frannie{Do we want to add the addition of the new node's label to the label vector here when we add a novel node?}
...
\begin{figure}
    ...
    \end{figure}
\frannie{Labels within the figure use old notation.  $Eta_+$, $Eta_-$ and $\lambda_+$ need to be changed to current notation}
    ...
    \frannie{Do $\opp{\copyrate{}}$ and $\opp{\extantrate{}}$ matter here?  Is the difference what's important?}
...
\subsection{Asymptotic Properties} \label{sec:asymptotics}
    ...
    \end{figure*}
    \frannie{Awkward sentence (visualization of the long-run dynamics of our model, for parameters) in caption.  Not sure what it is trying to say}
    \frannie{Typo in caption of panel (a)?  $\opp{\extantrate{}} = \opp{\novelrate{}}3$?  }
...
\subsubsection{Degree Distribution} \label{sec:degree-distribution}
    ...
    A derivation of this result and a computational illustration are provided in \Cref{sec:degree-distribution-appendix} and \Cref{fig:degrees}. 
