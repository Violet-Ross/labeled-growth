...
Each synthetic hypergraph is generated using our model (\violet{should the model have a name?}) from a known parameter vector $\paramvector$.
...
\begin{figure}[t]
    \includegraphics[width=0.5\textwidth]{../fig/sem_convergence_heatmap.png}
    \caption{Performance of SEM on synthetic hypergraphs. 
    Each entry in the heatmap corresponds to a set of parameters $\paramvector$, specified in Appendix \ref{sec:SEM-appendix}.
    For each set of parameters, five indepedent hypergraphs are generated according to the model and SEM is performed independently on each.
...
\section*{Data Availability}
    ...
    The code used to generate the results in this paper is available at [GITHUB LINK].
...
\appendix
    ...
    \begin{table}[h]
        \caption{Strength of Homophily in External and Novel Node Addition}
        \begin{tabular}{ c c c }
            \toprule
            & $\extantrate_+, \extantrate_-$ & $\novelrate_+, \novelrate_-$\\
            \midrule
            \makecell{Weak in external \\ node addition} & 1, 0.8 & \nullnov \\
            \makecell{Strong in external \\ node addition} & 2, 0.2 & \nullnov \\
            \makecell{Weak in novel \\ node addition} & \nullext & 1, 0.8 \\
            \makecell{Strong in novel \\ node addition} & \nullext & 2, 0.2 \\
            \makecell{Weak in both} & 1, 0.8 & 1, 0.8 \\
            \makecell{Strong in both} & 2, 0.2 & 2, 0.2 \\
            \bottomrule
        \end{tabular}
    \end{table}
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{ c c c }
            ...
            & $\extantrate_+, \extantrate_-$ & $\novelrate_+, \novelrate_-$\\
            \midrule
            \makecell{Both weak} & 0.1, 0.1 & 0.1, 0.1 \\
            \makecell{Strong external \\ node mechanism} & 2, 2 & \nullnov \\
            \makecell{Weak novel \\ node mechanism} & \nullext & 2, 2 \\
            \makecell{Both strong} & 2, 2 & 2, 2 \\
            \bottomrule
        \end{tabular}
    \end{table}
