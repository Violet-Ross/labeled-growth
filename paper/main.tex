...
To measure the success of a reconstruction, we compute the Kullback-Leibler (KL) divergence between the true and estimated distributions described by each of the six parameters, and add the results. 
Using standard results, our measure of error then becomes
\begin{align}
    D[p_{\paramvector} || \hat{p}_{\paramvector{}}] = 
\end{align}


\Cref{fig:synthetic_sem} demonstrates the accuracy of the SEM-inferred parameter estimates for each of these synthetic hypergraphs.
The heatmap shows that the divergence between true and estimated parameters varies depending on the true parameters.
Still, a visual inspection of SEM convergence on the hypergraph with poorest performance shows that the distance between the true and estimated parameter values is small.
...
\begin{figure*}[t]
    ...
    \caption{
        ...
        The col or of an entry in the heatmap represents the mean KL divergence over the five simulations for that set of parameters.
...
We performed SEM on six empirical higher-order networks, represented as hypergraphs. 
...
\begin{table}
    \caption{Community detection results ARI on empirical datasets.  Simulated annealing returns average ARI on 10 runs}\label{tab:results}
