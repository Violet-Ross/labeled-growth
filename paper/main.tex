...
\documentclass[aps,pre,reprint,superscriptaddress,nofootinbib,12pt,onecolumn]{revtex4-2}
...
\date{\today}
\begin{abstract}
    There are many extant models of hypergraphs with interactions governed by some form of homophily or assortativity, but most assume independence between edges conditional on node parameters. 
    Relaxing this assumption, we study a mechanistic model of growing hypergraphs in which edge formation is influenced by the realization of previous edges and is tuned by homophily between binary node labels. 
    ...
    This model defines a likelihood over a labeled hypergraph, allowing us to use standard maximum-likelihood techniques to structure algorithms. 
    ...
    Our findings highlight the benefits of moving beyond edge-independence assumptions in modeling and data analysis, and point to several directions for future work. 
\end{abstract}
...
\phil{clarify assortativity the structure vs. homophily the process throughout and say that's how we're using the terms.}
...
\section{Introduction}
    ...
    One then forms the edge set $\edges$ iteratively by visiting each subset $R \subseteq \nodes$ and independently assigning $a_R \in \mathbb{N}$ (possibly repeated) edges to $R$ according to a distribution $\lik(a_R | \vz_R, \theta)$, where $\vz_R$ is the vector of labels of the nodes in $R$ and $\theta$ is a set of model parameters.
    ...
    \begin{align}
        \lik(\hypergraph | \vz; \theta) = \prod_{R \subseteq \nodes} \lik(a_R | \vz_R; \theta) \;. \label{eq:edge-independence}
    \end{align}
    ...
    \phil{Need to standardize notation for likelihood: should it just be lowercase $p$ or something different and fancier?}
    ...
    To this end, we introduce a model with binary labels in which edges form via a noisy edge copy process tuned by the node labels within the copied edge.
    ...
    Our findings highlight the benefits of using explicit stochastic generative models to guide data analysis algorithms for studying homophily in polyadic systems.
...
\begin{figure}
    \includegraphics[width=0.49\textwidth]{../fig/community-structure-sims-combined.png}
        \caption{
...
\subsection{Asymptotic Properties} \label{sec:asymptotics}
    ...
    \begin{figure*}[t]
        ...
        \caption{
            ...
            \phil{Frannie and Violet both, I'd like some feedback from you on whether this figure and accompanying discussion seem valuable in proportion to the space they occupy in the paper so far.}
        }
...
\subsubsection{Degree Distribution} \label{sec:degree-distribution}
    ...
    \begin{align}
        \zeta &= 1 + \frac{\brackets{k_0} + \brackets{k_1}}{1 + \same{\copyrate{}}(\nu_{00} + \nu_{11} - 1) + 2\opp{\copyrate{}} \nu_{01}} \label{eq:power-law-exponent}\\ 
        \nu_{ij} &= \brackets{\frac{k_i k_j}{k}} \;. \nonumber
    \end{align}
    In the expression for $\nu_{ij}$ (and for $\brackets{k_i}$) the expectation is taken with respect to the stationary joint distribution $\jointdist \paren{k_0, k_1}$.
...
\subsection{Parameter Estimation via Stochastic EM} \label{sec:sem}
    ...
    \frannie{Might be helpful to broaden $\prec$ to $t_f < t_e$ and $|f\cap e| \neq 0$. Would incorporate logic in note after \cref{eq:bayes}}
...
We perform SEM on a set of hypergraphs that were generated with varying amounts of homophily across each of the three growth mechanisms: the edge copy step, the extant node addition step, and the novel node addition step.
...
\begin{align}
    D[p(\cdot ; {\paramvector}) || p(\cdot ; \hat{\paramvector{}})] &= \phantom{+}\sum_{* \in \{+, -\}} c_*\left( \copyrate_{*} \log \frac{\copyrate_{*}}{\hat{\copyrate}_{*}} +  (1 - \copyrate_{*}) \log \frac{1 - \copyrate_{*}}{1 - \hat{\copyrate}_{*}} \right) \nonumber\\ 
...
The \texttt{senate-bills} dataset appears to have slight homophily in the estimates of the edge copy and extant node addition parameters.
...
For high levels of homophily (larger values of the varied parameter), simulated annealing under the Nishimori condition can nevertheless be outperformed by Laplacian spectral clustering. 
Notably, however, there are regions of parameter space, especially those with low levels of homophily (smaller values of the varied parameter), in which both simulated annealing under the fixed condition and under the Nishimori condition outperform Laplacian spectral clustering. 
This result is also unsurprising, since we perform our experiment on synthetic samples drawn directly from the \model{} model. 
...
\section{Discussion}
    ...
    The copying, extant, and novel node addition steps are all tuned by node labels and parameters which control the strength of homophily or heterophily in each of these steps.
    We have demonstrated efficient parameter inference in the \model{} model via stochastic expectation maximization (SEM). 
    We have also demonstrated that, in some circumstances, maximum-likelihood inference of node labels $\vz$ via simulated 
    annealing can outperform both dyadic and recent polyadic community detection algorithms that proceed from the assumption of conditional edge independence. 
...
\appendix
    ...
    \end{align}
    Similarly, the expected number of nodes that are sampled in the extant node addition step is $\extantrate{} \triangleq \same{\extantrate{}} + \opp{\extantrate{}}$. 
    ...
    Explicitly calculating this exponent requires the moments $\brackets{k}$, $\rho_{00}$, $\rho_{11}$, and $\rho_{01}$, which can be computed from the stationary joint distribution $\at{\jointdist}{\infty} \paren{k_1,k_2}$. 
    ...
    \subsection{Sufficient Statistics}
        ...
        \begin{align*}
            ...
            \end{pmatrix}^\top \;.
        \end{align*}
        As descrbed in \Cref{sec:sem}, the vector $\sufficientstatsvec$ of expected sufficient statistics is computed as a weighted average of $\sigma(e, f, u, \vz)$ over the belief distribution $\lik(f, u | e, \at{\hat{\paramvector}}{\ell})$ over the latent variables $f$ and $u$.
    ...
    \subsection{Implementation Details}
        ...
        We use an exponentially decaying learning rate for SEM with form $\lr^{(t)} = \lr^{(0)} \cdot e^{-ct}$.
        The initial learning rate is set to $\lr^{(0)} = 0.01$, and the exponent is defined as $c = 0.001$.
    ...
    \section{Simulated Annealing for Community Detection}\label{sec:simulated-annealing-algorithm-spec}
        ...
        Let $\atalg{\acceptprob}{\epoch}$ be the probability that the simulated annealing algorithm accepts a change on labels that lowers the approximated likelihood
        
        \begin{algorithm}[H]
            \begin{algorithmic}
                \caption{\texttt{SimulatedAnnealing}$(\hypergraph, \theta)$}\label{algocf:simulated_annealing}
                \Require{Hypergraph $\hypergraph$, parameters $\paramvector$, number of epochs $E$}
                \Ensure{Label set $\vz$, approximation of the true labels $\vz^*$}
                \State Sample $\vz \sim \text{Uniform}(\{0,1\})^{n}$ \Comment{initialization}
                ...
                \For{$\epoch = 1$ to $E$}
                    \For{$\step = 1$ to $n$}
                        ...
                        \State $\Delta p \gets \log \tilde{\lik}(\hypergraph, \vz^\prime | \paramvector) - \log \tilde{\lik}(\hypergraph, \vz | \paramvector)$
                        \If{ $\epoch = 0$}
                            \State $\Delta \mathcal{L} \gets \Delta \mathcal{L} \cup \{\Delta p\}$
                        \EndIf
                        ...
                        \Else
                            \State $\atalg{\acceptprob}{\epoch}$ $\gets$ 1
                            \State $\zeta_a$ $\gets$ $\Delta E/\sigma_\hypergraph$
                            \If{$1 \leq \epoch \leq 4$}
                                \State $\atalg{\acceptprob}{\epoch}$ $\gets$ $\mathrm{Gaussian}(\zeta_a;\mu = 0, \sigma=2$)
                            \ElsIf{5 $\leq \epoch$}
                                \State $\atalg{\acceptprob}{\epoch}$ $\gets$ $\mathrm{Gaussian}(\zeta_a;\mu = 0, \sigma=2(1 - \frac{\epoch}{A}))$
                            \EndIf
    ...
    \section{Simulated Annealing Illustration}
        ...
        \frannie{What does nats per edge mean?}
        ...
        \end{align*}
        \frannie{Params are the same here, should we just say they are used to generate the hypergraph and recover the community structure?}
    
    \section{More Simulated Annealing on Synthetic Data}
    \frannie{Empty as is.  I liked the heatmap, but understand if it doesn't justify its space!}
...
% \end{figure*}
    ...
    \frannie{Fix formatting here.  Not sure if the section is finished as is}
