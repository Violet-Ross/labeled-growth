...
Each synthetic hypergraph is generated using our model (\violet{should the model have a name?} \phil{Yes, an ideal name will have an acronym which is fun in some way.}) from a known parameter vector $\paramvector$.
We performed SEM accross a set of hypergraphs which were generated from varying amounts of homophily in each step of the growth process as well as variation in the expected number of nodes added to $e$ in each step.
...
\begin{figure*}[t]
    ...
    \caption{
        ...
        Each entry in the heatmap corresponds to a set of parameters $\paramvector$, specified in Appendix \ref{sec:SEM-appendix}.        For each set of parameters, five indepedent hypergraphs are generated according to the model and SEM is performed independently on each.
        ...
        Details on the parameters used in these simulations can be found in  \Cref{sec:SEM-appendix}, \Cref{tab:sem-heatmap-params}. 
    }
...
The coauthorship data, meanwhile, falls on the line of equality for the edge copy and external node addition mechanisms, but is plotted below the line of equality in the nodel node addition panel.
