...
\section{Introduction}
    ...
    One then forms the edge set $\edges$ iteratively by visiting each subset $R \subseteq \nodes$ and independently assigning $a_R \in \mathbb{N}$ (possibly repeated) edges to $R$ according to a distribution $p(a_R | \vz_R, \theta)$, where $\vz_R$ is the vector of labels of the nodes in $R$ and $\theta$ is a set of model parameters.
...
\section{The \model{} Model} \label{sec:model-description}
    ...
    \begin{itemize}
        ...
        \item 
            \textbf{Extant Node Addition}: We sample $\extantnodesinedge_{z_\focalnode} \sim \poisson{\same{\extantrate{}}}$ nodes from $\nodes^t \setminus \seededge$ with label $z_\focalnode$ and add them to $\newedge$. 
            We similarly sample $\extantnodesinedge_{\bar{z}_\focalnode} \sim \poisson{\opp{\extantrate{}}}$ nodes from $\nodes^t \setminus \seededge$ with label $\bar{z}_\focalnode$ and add them to $\newedge$.
...
\subsection{Parameter Estimation via Stochastic EM} \label{sec:sem}
    ...
    \begin{align}
        \prob(\hypergraph, \vz | \paramvector) = \prod_{\newedge \in \edges} \sum_{\seededge \prec \newedge} \sum_{\focalnode \in \seededge} p(\newedge | \seededge, \focalnode, \vz, \paramvector) p(\seededge,\focalnode| \edges^{t - 1}, \vz, \paramvector) \;. \label{eq:likelihood}
    \end{align}
    For notational simplicity, we have suppressed the additional entries of $\vz$ introduced by the novel node addition step. 
    ...
    Each iteration of the SEM algorithm consists of two steps: the E-step, in which exponentially weighted moving averages of the sufficient statistics are calculated using the current parameter estimates, and the M-step, in which the parameter estimates are updated using the expected sufficient statistics. 
...
Then, given current estimates $\at{\hat{\paramvector}}{\ell}$, we form a belief distribution over the latent identities of $\seededge \in \edges$ and $\focalnode \in \seededge$ which could have been selected in the edge-copying step to form $\newedge$. 
...
\begin{algorithm}[H]
    ...
    \For{$\seededge \in \edges$}
        \If{$\seededge \prec \newedge$}
            \For{$\focalnode \in \newedge \cap \seededge$}
                \State $a \gets a + p(\seededge, \focalnode | \newedge, \at{\hat{\paramvector}}{\ell}) \sigma(\newedge, \seededge, \focalnode, \vz)$
                \State $b \gets b + p(\seededge, \focalnode | \newedge, \at{\hat{\paramvector}}{\ell})$
            \EndFor
        \EndIf
    \EndFor
...
\begin{align*}
    h(\at{\hat{\paramvector}}{\ell}, \at{\hat{\paramvector}}{\ell'}) = \min_{i} \left\{\abs{\at{\hat{\param}_i}{\ell} - \at{\hat{\param}_i}{\ell'}} - \epsilon_\mathrm{rel} - \epsilon_\mathrm{abs} \at{\hat{\param}_i}{\ell'}\right\} \;.
\end{align*}
...
In our implementation, we declare convergence at timestep $\ell$ when $h(\at{\hat{\paramvector}}{\ell}, \at{\hat{\paramvector}}{\ell'}) \geq 0$ for all $\ell'$ in the most recent 800 iterations of the algorithm.
In our experiments we used $\epsilon_\mathrm{rel} = 0.01$ and $\epsilon_\mathrm{abs} = 0.015$.
...
\subsection{Community Detection via Simulated Annealing} \label{sec:community-detection}
    ...
    To mitigate the computational burden, we use an intersection-based approximation to the likelihood of forming edge $\newedge$ in which we only sum over candidate copied edges $\seededge$ such that $\seededge = \argmax_{\seededge' \in \edges} \abs{\seededge' \cap \newedge}$.
    To justify this approximation, we start by observing that, if $\seededge$ is the edge sampled to form $\newedge$, then the number of extant nodes required to form $\newedge$ is $x = \abs{\newedge} - \abs{\newedge\cap \seededge} - \abs{\newedge \setminus \nodes}$. 
    We will argue that, when the number of nodes $n$ is large, $p(\newedge|\seededge, \focalnode, \vz, \paramvector) \in O(n^{-x})$, that implies that edges $\seededge$ which do not maximize $\abs{\seededge \cap \newedge}$ will have negligible contributions to the likelihood.
    The essence of the argument is that forming $x$ extant nodes requires $x$ independent samples from the sets $\nodes_0\setminus \seededge$ and $\nodes_1\setminus \seededge$, each of which has size scaling linearly with $n$. 
    So, each independent sample has probability $O(n^{-1})$, and the probability of forming a given set of $x$ extant nodes is $O(n^{-x})$.
    We detail this argument in \Cref{sec:simulated-annealing-approximation}, obtaining an approximate objective function for simulated annealing 
    \begin{align*}
        \tilde{\cL}(\vz) = \log \tilde{\prob}(\hypergraph, \vz | \paramvector) \;,
    \end{align*}
    where $\tilde{\prob}(\hypergraph, \vz | \paramvector)$ is the likelihood of $\hypergraph$ and $\vz$ under the intersection-based approximation.
    To run simulated annealing for a single epoch, we perform $n$ individual update steps, where $n$ is the number of nodes in the considered $\hypergraph$. 
    In each step, we propose a change to the labels $\vz$ by generating a node index $j$ uniformly at random and evaluating the change in likelihood of "flipping" the binary node label ($z_j \gets \bar{z}_j$).  
    If the proposal increases $\tilde{\cL}(\vz)$, it is accepted.
    If the proposal decreases $\tilde{\cL}(\vz)$, however, we may also accept it with an acceptance probability $a(\epoch)$ that decreases with the epoch $\epoch$ and considers the magnitude of the decrease in $\tilde{\cL}(\vz)$.
    ...
    This results in a roughly 30-fold reduction in compute time on our empirical data sets at the expense of increasing memory requirements. 
    
    We use a custom acceptance schedule $a(\epoch)$ for our simulated annealing implementation.
    ...
    For the remaining epochs, we accept proposals that decrease $\tilde{\cL}$ by an amount $\Delta \tilde{\cL}$ stochastically by evaluating quantiles of a Gaussian distribution with mean 0 and a standard deviation proportional to $\sigma(\Delta \tilde{\cL})$, which decays with epoch. 
...
Each synthetic hypergraph is generated using our model and a known parameter vector $\paramvector$.
We performed SEM on a set of hypergraphs that were generated with varying amounts of homophily across each of the three growth mechanisms: the edge copy step, the extant node addition step, and the novel node addition step.
...
The \texttt{high-school} and \texttt{primary-school} datasets \cite{gemmetto2014mitigation,stehle2011high,mastrandrea2015contact} are parsed into hypergraphs of social interactions between students in a high school and primary school, respectively, with node labels corresponding to the gender of each student; these data sets were collected through the SocioPatterns project.
Due to the high temporal resolution of these data, we consider a sequence of interactions to correspond to a single hyperedge if those interactions are identical and temporally contiguous. 
...
\begin{table*}
    \caption{Data sets used in this paper. We show the number of nodes $n$, edges $m$, average degree $\langle d \rangle$, and average edge size $\langle k \rangle$ for each dataset.}\label{tab:datasets}
...
However, the other Democratic cosponsors of $\seededge$ are not indebted in the same way to $\focalnode$, perhaps making them less likely to cosponsor $\newedge.$
...
The biggest difference in our estimates for these two systems is in the extant node addition parameter estimates; the \texttt{house-bills} data set has estimates $\same{\hat{\extantrate}}$ and $\opp{\hat{\extantrate}}$ which are each roughly three times larger than their values in the \texttt{senate-bills} data set.
...
And since 73\% of edges in the hypergraph have size 2, and less than one percent of the nodes in the hypergraph are core nodes, any given edge likely contains one core node and one fringe node.
...
These experiments do suggest, however, that there exist classes of hypergraphs in which community structure can be more reliably detected via likelihood maximization under \model{} than by at least this standard graph method. 

We can see this phenomenon in experiments on empirical data as well. 
...
We also include as comparisons two dyadic methods, implemented on the weighted clique projection of the hypergraph, that allow the user to specify the number of clusters: Laplacian spectral clustering \cite{luxburg2007tutorial} and greedy modularity maximization \cite{clauset2004finding}.
...
In \Cref{fig:community-empirical}, we observe that \model{} simulated annealing outperforms both greedy-modularity and spectral clustering on senate bills in all 10 runs in ARI.  
...
While the computational performance of simulated annealing presents a difficulty in its application, the detection of communities via the maximization of likelihood provides evidence that the formations of these datasets, especially senate-bills, can be simulated via our proposed model $\model$.
Further, the success of our model in comparison to spectral clustering and greedy modularity provides evidence challenging the assumption of edge independence within \texttt{senate-bills}.
This is because the baseline methods, greedy modularity and spectral clustering, are frequently justified via the stochastic block model \cite{lei2015consistency, newman2016equivalence}, which relies on the assumption of edge indepenece, where our method explicitly utilizes dependent hyperedges.
\frannie{Is this paragraph left over, or did you want to keep this Phil?}
...
% \phil{Good idea to look on this. Spectral clustering is not a generative (probabilistic model-based) method, but many of its standard justifications do rely on stochastic block models which have the independence assumption. The best citation is probably this one: \cite{lei2015consistency}}. 
...
\section{Discussion}
    ...
    We have proposed \Model{} (\model{}), a generative model for the formation of binary-labeled hypergraphs in which edges form as noisy copies of previous edges, combined with the addition of extant and novel nodes.
    ...
    annealing can outperform both dyadic and polyadic community detection algorithms that proceed from the assumption of conditional edge independence. 
    ...
    Most obviously, although our results show that maximum-likelihood inference in the \model{} model is promising for detecting certain kinds of community structure, the simulated annealing method we have used is impractically expensive on data sets of even relatively modest size. 
    ...
    We note, for example,  that \cite{chodrowNonbacktrackingSpectralClustering2023} proposed an algorithm that obtained an ARI of approximately 0.25 on the \texttt{senate-bills} data set, higher than our proposed simulated annealing method. 
    Their algorithm requires alternating between parameter estimation and community detection phases. 
    In principle, we could attempt a similar alternating procedure in our model, but the computational cost of simulated annealing makes this approach infeasible in practice.
...
\appendix
    ...
    \section{Simulated Annealing for Community Detection}\label{sec:simulated-annealing-algorithm-spec}
        ...
        \begin{algorithm}[H]
            \begin{algorithmic}
                ...
                \For{$\epoch = 0$ to $A$}
                    \For{$\step = 0$ to $n$}
                        ...
                        \Else
                            \State $a(\epoch)$ $\gets$ 1
                            \State $\zeta_a$ $\gets$ $\Delta E/\sigma_\hypergraph$
                            \If{$1 \leq \epoch \leq 4$}
                                \State $a(\epoch)$ $\gets$ Normal($\zeta_a|\mu = 0, \sigma=2$)
                            \ElsIf{5 $\leq \epoch$}
                                \State $a(\epoch)$ $\gets$ Normal($\zeta_a|\mu = 0, \sigma=2(1 - \frac{\epoch}{A}))$
                            \EndIf
            ...
            \end{algorithmic}
        \end{algorithm}
        \frannie{Can our $\Delta \mathcal{L}$ be a set like this?  Will this not accept duplicates?}
